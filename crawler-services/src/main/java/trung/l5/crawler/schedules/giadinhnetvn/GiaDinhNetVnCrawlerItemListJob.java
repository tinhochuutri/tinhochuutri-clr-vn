package trung.l5.crawler.schedules.giadinhnetvn;
...
import trung.l5.crawler.reader.giadinhnetvn.GiaDinhNetVnReader;
import trung.l5.crawler.services.AppModelService;

@DisallowConcurrentExecution
public class GiaDinhNetVnCrawlerItemListJob implements Job {
    ...
    int timeAgo;
    @Autowired
    GiaDinhNetVnReader GiaDinhNetVnReader;
    ...
    public void execute(JobExecutionContext jobExecutionContext) {
        LOGGER.info("* Bắt đầu crawler giadinh.net.vn ...");

        //
        List<String> pages = GiaDinhNetVnReader.getPages(startPage, endPage);
        for (String pageUrl : pages) {
            try {
                LOGGER.info("* Start crawling item of page " + pageUrl);
                List<AppModel> sourceWrapperModels = GiaDinhNetVnReader.readItemsOfPage(pageUrl);
