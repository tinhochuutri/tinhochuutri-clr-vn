package trung.l5.crawler.schedules.vnexpress;
...
import trung.l5.crawler.reader.vnexpress.VnExpressReader;
import trung.l5.crawler.services.AppModelService;

@DisallowConcurrentExecution
public class VnExpressCrawlerItemListJob implements Job {
    ...
    int timeAgo;
    @Autowired
    VnExpressReader vnExpressReader;
    ...
    public void execute(JobExecutionContext jobExecutionContext) {
        LOGGER.info("* Start crawler item list ...");

        //
        List<String> pages = vnExpressReader.getPages(startPage, endPage);
        for (String pageUrl : pages) {
            try {
                LOGGER.info("* Start crawling item of page " + pageUrl);
                List<AppModel> sourceWrapperModels = vnExpressReader.readItemsOfPage(pageUrl);
