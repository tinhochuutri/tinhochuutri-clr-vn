package trung.l5.crawler.schedules.tienphong;
...
import trung.l5.crawler.reader.tienphong.TienPhongReader;
import trung.l5.crawler.services.AppModelService;

@DisallowConcurrentExecution
public class TienPhongCrawlerListJob implements Job {
    ...
    int timeAgo;
    @Autowired
    TienPhongReader TienPhongReader;
    ...
    public void execute(JobExecutionContext jobExecutionContext) {
        LOGGER.info("* Bắt đầu crawler tienphong.vn ...");

        //
        List<String> pages = TienPhongReader.getPages(startPage, endPage);
        for (String pageUrl : pages) {
            try {
                LOGGER.info("* Start crawling item of page " + pageUrl);
                List<AppModel> sourceWrapperModels = TienPhongReader.readItemsOfPage(pageUrl);
