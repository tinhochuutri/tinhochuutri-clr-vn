package trung.l5.crawler.schedules.laodong;
...
import trung.l5.crawler.reader.laodong.LaoDongReader;
import trung.l5.crawler.services.AppModelService;

@DisallowConcurrentExecution
public class LaoDongCrawlerItemListJob implements Job {
    ...
    public void execute(JobExecutionContext jobExecutionContext) {
        LOGGER.info("* Bắt đầu crawler laodong.vn ...");

        //
        List<String> pages = LaoDongReader.getPages(startPage, endPage);
        for (String pageUrl : pages) {
            try {
                LOGGER.info("* Start crawling item of page " + pageUrl);
                List<AppModel> sourceWrapperModels = LaoDongReader.readItemsOfPage(pageUrl);
