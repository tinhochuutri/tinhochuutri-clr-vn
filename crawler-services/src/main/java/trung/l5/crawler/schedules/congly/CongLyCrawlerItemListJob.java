package trung.l5.crawler.schedules.congly;
...
import trung.l5.crawler.reader.congly.CongLyReader;
import trung.l5.crawler.services.AppModelService;

@DisallowConcurrentExecution
public class CongLyCrawlerItemListJob implements Job {

    private static final Logger LOGGER = LogManager.getLogger(CongLyCrawlerItemListJob.class);
    ...
    public void execute(JobExecutionContext jobExecutionContext) {
        LOGGER.info("* Bắt đầu crawler congly.vn ...");

        //
        List<String> pages = CongLyReader.getPages(startPage, endPage);
        for (String pageUrl : pages) {
            try {
                LOGGER.info("* Start crawling item of page " + pageUrl);
                List<AppModel> sourceWrapperModels = CongLyReader.readItemsOfPage(pageUrl);
