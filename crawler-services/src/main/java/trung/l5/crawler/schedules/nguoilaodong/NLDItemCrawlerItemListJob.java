package trung.l5.crawler.schedules.nguoilaodong;
...
import trung.l5.crawler.reader.nguoilaodong.NLDReader;
import trung.l5.crawler.services.AppModelService;

@DisallowConcurrentExecution
public class NLDItemCrawlerItemListJob implements Job {
    ...
    public void execute(JobExecutionContext jobExecutionContext) {
        LOGGER.info("* Bắt đầu crawler nguoilaodong.vn ...");

        //
        List<String> pages = NLDReader.getPages(startPage, endPage);
        for (String pageUrl : pages) {
            try {
                LOGGER.info("* Start crawling item of page " + pageUrl);
                List<AppModel> sourceWrapperModels = NLDReader.readItemsOfPage(pageUrl);
