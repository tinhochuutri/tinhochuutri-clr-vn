package trung.l5.crawler.schedules.danviet;
...
import trung.l5.crawler.reader.danviet.DanVietReader;
import trung.l5.crawler.services.AppModelService;

@DisallowConcurrentExecution
public class DanVietCrawlerItemListJob implements Job {
    ...
    public void execute(JobExecutionContext jobExecutionContext) {
        LOGGER.info("* Bắt đầu crawler DÂN VIỆT ...");

        //
        List<String> pages = DanVietReader.getPages(startPage, endPage);
        for (String pageUrl : pages) {
            try {
                LOGGER.info("* Start crawling item of page " + pageUrl);
                List<AppModel> sourceWrapperModels = DanVietReader.readItemsOfPage(pageUrl);
