package trung.l5.crawler.schedules.vietnamnet;
...
import trung.l5.crawler.reader.vietnamnet.VietNamNetReader;
import trung.l5.crawler.services.AppModelService;

@DisallowConcurrentExecution
public class VietNamNetCrawlerItemListJob implements Job {

    private static final Logger LOGGER = LogManager.getLogger(VietNamNetCrawlerItemListJob.class);
    ...
    int timeAgo;
    @Autowired
    VietNamNetReader VietNamNetReader;
    ...
    public void execute(JobExecutionContext jobExecutionContext) {
        ...
        //
        List<String> pages = VietNamNetReader.getPages(startPage, endPage);
        for (String pageUrl : pages) {
            try {
                LOGGER.info("* Start crawling item of page " + pageUrl);
                List<AppModel> sourceWrapperModels = VietNamNetReader.readItemsOfPage(pageUrl);
