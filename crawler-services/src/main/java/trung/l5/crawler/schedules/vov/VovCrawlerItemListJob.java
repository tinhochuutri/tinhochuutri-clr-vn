package trung.l5.crawler.schedules.vov;
...
import trung.l5.crawler.reader.vov.VovReader;
import trung.l5.crawler.services.AppModelService;

@DisallowConcurrentExecution
public class VovCrawlerItemListJob implements Job {
    ...
    public void execute(JobExecutionContext jobExecutionContext) {
        LOGGER.info("* Bắt đầu crawler vov ...");

        //
        List<String> pages = VovReader.getPages(startPage, endPage);
        for (String pageUrl : pages) {
            try {
                LOGGER.info("* Start crawling item of page " + pageUrl);
                List<AppModel> sourceWrapperModels = VovReader.readItemsOfPage(pageUrl);
