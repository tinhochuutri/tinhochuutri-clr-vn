package trung.l5.crawler.schedules.tuoitre;
...
import trung.l5.crawler.reader.tuoitre.TuoiTreReader;
import trung.l5.crawler.services.AppModelService;

@DisallowConcurrentExecution
public class TuoiTreCrawlerItemListJob implements Job {
    ...
    public void execute(JobExecutionContext jobExecutionContext) {
        LOGGER.info("* Bắt đầu crawler tuoitre ...");

        //
        List<String> pages = TuoiTreReader.getPages(startPage, endPage);
        for (String pageUrl : pages) {
            try {
                LOGGER.info("* Start crawling item of page " + pageUrl);
                List<AppModel> sourceWrapperModels = TuoiTreReader.readItemsOfPage(pageUrl);
