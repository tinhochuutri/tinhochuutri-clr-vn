package trung.l5.crawler.schedules.anninhthudo;
...
import trung.l5.crawler.reader.anninhthudo.AnNinhReader;
import trung.l5.crawler.services.AppModelService;

@DisallowConcurrentExecution
public class AnNinhCrawlerItemListJob implements Job {
    ...
    public void execute(JobExecutionContext jobExecutionContext) {
        LOGGER.info("* Bắt đầu crawler anhninhthudo.vn ...");

        //
        List<String> pages = AnNinhReader.getPages(startPage, endPage);
        for (String pageUrl : pages) {
            try {
                LOGGER.info("* Start crawling item of page " + pageUrl);
                List<AppModel> sourceWrapperModels = AnNinhReader.readItemsOfPage(pageUrl);
