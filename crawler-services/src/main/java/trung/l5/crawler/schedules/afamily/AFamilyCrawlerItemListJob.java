package trung.l5.crawler.schedules.afamily;
...
import trung.l5.crawler.reader.afamily.AFamilyReader;
import trung.l5.crawler.services.AppModelService;

@DisallowConcurrentExecution
public class AFamilyCrawlerItemListJob implements Job {
    ...
    public void execute(JobExecutionContext jobExecutionContext) {
        LOGGER.info("* Bắt đầu crawler afamily.vn ...");

        //
        List<String> pages = AFamilyReader.getPages(startPage, endPage);
        for (String pageUrl : pages) {
            try {
                LOGGER.info("* Start crawling item of page " + pageUrl);
                List<AppModel> sourceWrapperModels = AFamilyReader.readItemsOfPage(pageUrl);
